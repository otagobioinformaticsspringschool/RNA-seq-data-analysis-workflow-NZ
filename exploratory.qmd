...
From here and for the rest of the workshop, we will be working within R. 
...
coldata <- read.table("DataFiles/coldata.txt", sep="\t", header=TRUE)
...
Zero count genes are automatically removed during differential gene expression, so are not a problem for downstream analysis. But for visualisation we likley need to deal with them. There will also likely be genes that are expressed in some samples, but not in others, and the zeros can cause issues with plotting there too.  It is common to remove genes with very low counts, but there are different approaches to chosing the threshold. For now, when plotting genes we need to log2 the counts, after adding 1 to all counts so that we don't log zero (which will result in an error or infinite value).  
...
This is *not* a count of how many genes are expressed per sample, this is the distribution of gene expression in each sample.
