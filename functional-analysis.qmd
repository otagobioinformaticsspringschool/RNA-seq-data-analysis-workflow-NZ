...
::: {.callout-note collapse="true"}
# Don't have the res_F_vs_TPM object?
Pull in the data from the backup folder here:

```{r}
#| eval: false

load("backup-files/deseq2-FvsTPM.RData")  
...
geneLengths <-  read.delim("DataFiles/genelengths.txt", header = TRUE, sep = "\t")
geneOntologies <- read.delim("DataFiles/geneontologies.txt", header = TRUE, sep = "\t")
...
### Investigate the under-represented GO terms 
...
We strongly recommend that analyses should be repeated with different tools or options and you take the time to assess the impact of these choices. In some cases, you may compare two tools and the results will be very similar and you gain confidence. In other cases you will need to decide which path to take, but you will be aware of the impact of your decisions. This is especially important for DGE - we recommend performing analysis with two separate tools (*e.g.,* DESeq2 and edgeR/Limma) and comparing the differential gene list overlap. 
