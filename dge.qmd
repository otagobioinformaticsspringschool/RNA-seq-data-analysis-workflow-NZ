...
not striclty necessary, code wont break as deseq will auto convert to factors.

But in our case we want to set an *order* - which we shoudl do with factor

this is becuase F MT TPM are a chronological transition in state, and we want to be able to see this order reflected in our results. T

comes up in ggplot2 when tryign to get things to plot on x axis in a specific order.  
...
Here when we factorise the histology column we also specify **levels**. This is optional but necessary if your metadata has an inherant order to it. These samples represent a chronological transition in state (*i.e.,* gonads transition from female --> multiple distinct transitional states --> terminal phase male)  
