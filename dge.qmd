...
write.table(tt_TPM_vs_F, file = "results/tt_TPM_vs_F.tsv", sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

write.table(tt_MT_vs_F, file = "results/tt_MT_vs_F.tsv", sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

write.table(tt_TPM_vs_MT, file = "results/tt_TPM_vs_MT.tsv", sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
```
...
sigGenesLimma_TPM_vs_F <- tt_TPM_vs_F[sigGenesLimma_TPM_vs_F, ]
```


::: {.callout-warning collapse="true" appearance="minimal"}
# 🌟 Bonus! Subset out all differentially expressed genes with abs()

Above we subsetted out genes that are upregulated in TPM (vs F), but sometimes we want to grab the list of ALL differentially expressed genes in a single comparison, regardless of which direction they are DE. 

We can do this using the `abs()` function, which takes the absolute value of logFC before filtering. This keeps genes with logFC > 1 OR logFC < -1, effectively excluding genes with a fold change between -1 and 1 (*i.e.,* not a substantial enough change in either direction). 


```{r}

# All DE genes using abs()
...
# test that our 'up' and 'down' lists combined are equal our 'all DE' list in number
...
dir.create("plots")
ggsave("plots/volcano_plot_tt_TPM_v_F.png", width = 10, height = 8, dpi = 300)
```
...
write.table(res_F_vs_TPM_df,  file = "results/deseq2_F-vs-TPM-results.tsv", sep = "\t", quote = FALSE, row.names = TRUE)
```
...
n_limma_total <- nrow(sigGenesLimma_TPM_vs_F)
n_deseq_total <- nrow(resPadjLogFC_F_vs_TPM)

```

In our run-through, Limma identified `r n_limma_total ` differentially expressed genes, while DESeq2 identified `r n_deseq_total` genes.
...
# Hello again future instructors! There is a back up file in results/ that was generated with:

save(res_F_vs_TPM, file = "results/deseq2-FvsTPM.RData") 
save(dds, file = "results/dds.RData")

# The dimensions of which are currently:
[1] 23773     6

# It was generated using DESeq2
> packageVersion("DESeq2")
[1] ‘1.52.0’

# If the package/dim changes, you may want to regenerate that file. It's mostly just there as a back-up for the next section.

# And while I'm here - the package verisons for limma and edge R are:
