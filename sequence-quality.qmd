...
# Sequence Quality - FastQC and Trimmomatic
...
![FastQC Per base sequence quality summary](images/per_base_quality.png)
...
# For reference: R1 and R2 'Per base sequence content' images -->

![R1 Per base sequence content](images/R1_perbaseseqcontent.png)

![R2 Per base sequence content](images/R2_perbaseseqcontent.png)


:::


1. Why does one fail and one have a warning? Read the [FastQC documentation](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) (Click on '3 Analysis Modules') and find the cut-offs that determine how these categories are assigned.

2. What is a likely explanation for the fail and warning for both reads? *Hint: there's a pattern you can see at the 5` end in both of the fastqc plots for these read files.*
...
The [FastQC documentation](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/) also shows an example of Bad Illumina Data. Navigate to it from the website or [open the report here.](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/bad_sequence_fastqc.html)
...
1. The bad data shows much greater spread in quality scores across base positions. The boxplots extend well down into the low Q score range (red range is Q0-Q20, indicating poor quality), particularly toward the 3' end of reads. In our data, quality remains consistently high across all positions, with very tight boxplots sitting in the green area (Q28+, very good quality) throughout.
...
Adapters are short, known sequences that can become embedded in your reads as part of the sequencing process. Before we work with our reads we want to remove these adaptors. Because adapters are manually added to the sequencing reaction, we should know exactly what these sequences are. In our examples, we have libraries generated with the Illumina TruSeq library prep kit, which uses standard [Illumina adapter sequences](https://sapac.support.illumina.com/downloads/illumina-adapter-sequences-document-1000000002694.html). 
...
> Tip: It is recommended to re-assess your fastq quality after trimming, by running FastQC on the trimmed files to and compare to pre-trimming.  
