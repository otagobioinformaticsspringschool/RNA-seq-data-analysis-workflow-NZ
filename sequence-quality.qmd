...
See the full [User guide](https://cutadapt.readthedocs.io/en/stable/guide.html) to learn more about Cutadapt.
...
get learners to do fastqc on raw reads
then run on trimmed reads and copy output to multiqc folder and run multiqc. 
then copy star align final out to multiqc folder and run again. will have two reports. 

supply to them the real reports for multiqc on trim only, and multiqc on trim and star. 
supply to them real report for one fastqc data. (R1 and R2). can explore bad data example on fastqc website. Thier reads are 100k which i think thats what fastqc looks at any way jst the first 100k. 
